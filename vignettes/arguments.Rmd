...
To avoid the occurrence of zero probabilities in the softmax function—which would lead the log-likelihood to diverge to negative infinity (`log(P) -> log(0) -> -Inf`)—we imposed a lapse rate of 0.02. In TAFC tasks, this adjustment guarantees that each option retains a minimum selection probability of 1%.
...
\log P(\theta) = 
\begin{cases} 
   \log P(\theta | \mu_0, \sigma_0^2) & 
   \quad \theta \sim \mathcal{N}(\mu_0, \sigma_0^2) 
   \\
   \log P(\theta | \lambda) & 
   \quad \theta \sim \text{Exp}(\lambda) \quad  
\end{cases}
$$
...
*NOTE: If `estimate = "MAP"` is used instead of `estimate = "MLE"`, the function `run_m` automatically returns the Log Posterior Probability in place of the Log-Likelihood (LL).*
