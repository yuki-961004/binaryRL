...
>  
> The final optimal parameters are influenced by several factors (ordered by importance):  
> 1. **Estimation Method**: MLE, MAP, MCMC, ABC, RNN
> 2. **Algorithm Package**: L-BFGS-B, GenSA, GA, DEoptim, PSO, Bayesian, CMA-ES, NLOPT)  
> 3. **Policy**: on vs off  
> 4. **Number of iterations**  
>  
> The following results are for demonstration only, based on:  
> - `estimate = "MLE"`  
...
recovery <- binaryRL::rcv_d(
  ...
  upper = list(c(1, 1), c(1, 1, 1), c(1, 1, 10)),
  iteration_s = 100,
  iteration_f = 100,
  nc = 1,
  algorithm = c("NLOPT_GN_MLSL", "NLOPT_LN_BOBYQA")
...
The value of the inverse temperature parameter ($\tau$) affects the recovery of other parameters in the model. Wilson & Collins [(2019)](https://doi.org/10.7554/eLife.49547) recommend increasing $\tau$ by 1, as this can help reduce the amount of noise in behavior. As our results show, Under the assumption that $\tau \sim \text{Exp}(1)$, adding 1 to all $\tau$ values improves the recovery of the learning rate  $\eta$, but decreases the recovery accuracy of  $\tau$ itself. Only estimation methods that bypass the log-likelihood, such as [ABC](https://yuki-961004.github.io/binaryRL/articles/ABC.html) and [RNN](https://yuki-961004.github.io/binaryRL/articles/RNN.html), seem to achieve excellent simultaneous recovery for both parameters.
...
- If `policy = "on"`, the model makes a choice based on its estimated probabilities and then updates the value of the chosen option based on its own selection. 
- If `policy = "off"`, the model only estimates the probabilities of different choices but does not make a choice itself. In this case, it always updates the value of the option chosen by the human. The accuracy (ACC) is meaningless in this scenario (it will always be 100%).

#### Visualizing Model Comparison (on-policy)

<p align="center">
    <img src="./demo/FIGURE/on-policy/model_comparison(abs).png" alt="RL Models" width="45%" style="display: inline;">
    <img src="./demo/FIGURE/on-policy/model_comparison(rel).png" alt="RL Models" width="45%" style="display: inline;">
</p>

#### Visualizing Model Comparison (off-policy)

<p align="center">
    <img src="./demo/FIGURE/off-policy/model_comparison(abs).png" alt="RL Models" width="45%" style="display: inline;">
    <img src="./demo/FIGURE/off-policy/model_comparison(rel).png" alt="RL Models" width="45%" style="display: inline;">
</p>
...
#### Visualizing Experimental Effect (on-policy)

<p align="center">
    <img src="./demo/FIGURE/on-policy/Exp_Effect(Frame).png" alt="RL Models" width="45%" style="display: inline;">
    <img src="./demo/FIGURE/on-policy/Exp_Effect(Block).png" alt="RL Models" width="45%" style="display: inline;">
</p>

#### Visualizing Experimental Effect (off-policy)

<p align="center">
    <img src="./demo/FIGURE/off-policy/Exp_Effect(Frame).png" alt="RL Models" width="45%" style="display: inline;">
    <img src="./demo/FIGURE/off-policy/Exp_Effect(Block).png" alt="RL Models" width="45%" style="display: inline;">
</p>
